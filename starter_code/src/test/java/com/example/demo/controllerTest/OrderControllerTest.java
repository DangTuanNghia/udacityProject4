...
public class OrderControllerTest {
    ...
    @Test
    public void testCreateOrderByUsername() {
        ...
        ResponseEntity<?> responseOder = orderController.submitOrder("test user name 1");
    ...
    public void testCreateOrderFailed() {
        User userMock = new User();
        userMock.setUsername("test user name 1");
        when(userRepository.findByUsername("test user name 1")).thenReturn(null);
    ...
    @Test
    public void testGetOrderValidUser() {
        User userMock = new User();
        userMock.setUsername("test user name 1");
        when(userRepository.findByUsername("test user name 1")).thenReturn(userMock);
        ...
        ResponseEntity<List<UserOrder>> responseOder = orderController.getOrdersHistory("test user name 1");
        assertNotNull(responseOder);
        assertEquals(200, responseOder.getStatusCodeValue());
    }

    @Test
    public void testGetOrderInvalidUsername() {
        User userMock = new User();
        userMock.setUsername("invalid username");
        when(userRepository.findByUsername("invalid username")).thenReturn(null);

        ResponseEntity<List<UserOrder>> responseOder = orderController.getOrdersHistory("invalid username");
        assertNotNull(responseOder);
        assertEquals(404, responseOder.getStatusCodeValue());
    }
}
